...
func (a *App) Run(ctx context.Context) error {
	...
	if !loggedIn {
		...
		qrMode := strings.ToLower(strings.TrimSpace(os.Getenv("QZONE_HISTORY_QR_MODE")))
		if qrMode == "" {
			qrMode = "browser"
		}
		...
		if qrMode == "terminal" {
			log.Println("在终端中显示二维码...")
			if err := qrcode.SaveAndDisplayQRCode(qrCode); err != nil {
				return logError("显示二维码失败", err)
			}
		} else {
			log.Println("在浏览器中打开二维码...")
			shutdownServer, notifySuccess, err = qrcode.OpenInBrowser(qrCode)
			if err != nil {
				log.Printf("打开浏览器失败: %v，尝试改为终端显示二维码...", err)
				if err2 := qrcode.SaveAndDisplayQRCode(qrCode); err2 != nil {
					return logError("显示二维码失败", err2)
				}
			}
